...
def main(args):
    ...
    text_inputs = torch.cat([
        clip.tokenize(f"a photo of a {c}") for c in args.class_names
    ]).to(device)
    ...
    classifier = ClipClassifier(clip_model,
                                args.class_names,
                                num_classes,
                                learn_prompt=args.learn_prompt,
                                bottleneck=None,
    ...
    optimizer = SGD(classifier.get_parameters(prompt_lr=args.prompt_lr),
                    args.lr,
    ...
    for epoch in range(args.epochs):
        ...
        train(train_target_iter, classifier, optimizer, lr_scheduler, epoch,
              args)
...
def train(train_target_iter: ForeverDataIterator, model: ClipClassifier,
...
if __name__ == '__main__':
    parser = argparse.ArgumentParser(
        description='Source Only for Unsupervised Domain Adaptation')
    # dataset parameters
    ...
    parser.add_argument('--lambda-im',
                        default=0.3,
                        type=float,
                        help='the trade-off hyper-parameter for im loss')
    parser.add_argument('--learn-prompt',
                        action='store_true',
